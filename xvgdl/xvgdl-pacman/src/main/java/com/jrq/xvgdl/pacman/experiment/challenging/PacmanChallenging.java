...
import com.jrq.xvgdl.pacman.experiment.challenging.PacmanChallengingSolution;
...
/**
 * This class is in charge of launching the challenging algorithm for Pacman game.
 *
 ...
 * - Passed Level: boolean -> false better than true
 * - Number of items left: integer -> Minimize
 * - Number of lives left: integer -> Minimize
 * - Gamplay time: long (milliseconds) -> Minimize difference with game timeout
 */
@Slf4j
public class PacmanChallenging {
    ...
    private static final int MAXIMUM_EVALUATIONS = 1000;

    private PacmanExperimentContextBuilder pacmanExperimentContextBuilder;
    private PacmanChallengingSolution bestSolution;
        ...
        try (FileWriter fileWriter = new FileWriter("/tmp/pacman_sol_" + System.currentTimeMillis() + ".csv");) {
            ...
            while (!finished) {
                ...
                for (PacmanExperimentConfiguration conf: configurationAndNeighbours) {
                    ...
                    PacmanChallengingSolution aSolution = runOptimization(
    ...
    public PacmanChallengingSolution runOptimization(int timeout, int numberOfGhosts, int numberOfBigDots, int numberOfLives, int percentageOfDots) {
        ...
        try {
            ...
            gameEngine.start();
            return PacmanChallengingSolution.buildFromContext(gameEngine.getGameContext());
