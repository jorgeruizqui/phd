...
@Slf4j
public class PacmanGenerationalGeneticProgramming {
    ...
    public void configureGeneticAlgorithmAndRun() {
        ...
        while (numberOfEvaluations < MAX_EVALUATIONS) {
            generateNewPopulationGenerationalApproach();
            playPopulation();
        }
        logResults();
    }
    ...
    private void playPopulation() {
        population.stream().forEach(gameContext -> {
    ...
    private void generateNewPopulationGenerationalApproach() {
        List<GameContext> newPopulation = new ArrayList<>();
        ...
        while (newPopulation.size() < POPULATION_SIZE) {
            ...
            newPopulation.add(generateIndividual(rulesMutated));
        }
