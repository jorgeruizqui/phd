...
public class PlayerNextMoveComparator {
    ...
    public List<ItemMovementSolution> orderedSolutions() {
        ...
        // Filtering 4 nearer items to avoid long searchs
    ...
    private List<String> getBasicMove(GameContext gameContext, IGameObject player, IGameObject item) {
        ...
        if (player.getPosition().getX() != item.getPosition().getX()) {
            if (player.getPosition().getX() > item.getPosition().getX()
                && (gameContext.getObjectAt(player.getPosition().getX() - 1, item.getPosition().getY(), 0) == null
                || !GameObjectType.WALL.equals(
                gameContext.getObjectAt(player.getPosition().getX() - 1, item.getPosition().getY(), 0)
                    .getObjectType()))) {
                newX--;
                moved = true;
            } else if (gameContext.getObjectAt(player.getPosition().getX() + 1, item.getPosition().getY(), 0) == null
                || !GameObjectType.WALL.equals(
                gameContext.getObjectAt(player.getPosition().getX() + 1, item.getPosition().getY(), 0)
                    .getObjectType())) {
                newX++;
        ...
        if (!moved) {
            if (player.getPosition().getY() != item.getPosition().getY()) {
                if (player.getPosition().getY() > item.getPosition().getY()
                    && (gameContext.getObjectAt(player.getPosition().getX(), item.getPosition().getY() - 1, 0) == null
                    || !GameObjectType.WALL.equals(
                    gameContext.getObjectAt(player.getPosition().getX(), item.getPosition().getY() - 1, 0)
                        .getObjectType()))) {
                    newY--;
                } else if (
                    gameContext.getObjectAt(player.getPosition().getX(), item.getPosition().getY() + 1, 0) == null
